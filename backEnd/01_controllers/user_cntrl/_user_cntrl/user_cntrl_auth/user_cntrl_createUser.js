import { user_srv_createUser } from "../../user_srv/_user_cntrl_srv.index.js";
import {
  consoleLog,
  debug_msg,
  validRespond_cntrl,
} from "../../../../06_helpers/_helpers.index.js";

const displayName = "user_cntrl_createUser.js";
const isDebug = true;
const service_Call = (req) => user_srv_createUser(isDebug, req);
const DEBUG_LOG = (m, e) => consoleLog("cntrl", displayName, isDebug, m, e);

const user_cntrl_createUser = async (req, res) => {
  DEBUG_LOG(debug_msg.start);
  try {
    const { success, message, data } = await service_Call(req);

    return validRespond_cntrl(success, message, data, DEBUG_LOG, res);
  } catch (error) {
    DEBUG_LOG(debug_msg.error_E, error);
  } finally {
    DEBUG_LOG(debug_msg.end);
  }
};

export default user_cntrl_createUser;
